...
다만 지금 프로즈 데이터에선 N=3 검증 PPL 이 **3.46** 로, 2토큰(3.02)보다 오히려 **높아요** —
데이터가 쉽고 작아 넓은 문맥의 이득이 안 나오거든요(진짜 이득은 데이터를 키우는 v0.4 에서).
조기 종료는 **학습 손실(Loss) 기준, patience=20**으로 돌아, 검증 최적점을 조금 지나쳐 학습해요.
