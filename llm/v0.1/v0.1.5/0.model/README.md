...
**새 개념:** 모델이 아니라 **'비교'** — 여러 버전을 같은 자로 나란히 재는 캡스톤.
...
모든 버전이 **같은 토크나이저(`punct+josa`) · 같은 `prepare`(`<END>`) · 같은 `perplexity()` 공식**을 쓰고,
...
- 대결표는 `2.test/test.py` 가 출력합니다:
  ...
  v0.0.9   카운트   ...    2.65    ...배   (앞 2토큰)
  v0.1.3   신경망   ...     ...     ...배    (앞 1토큰)
  v0.1.4   신경망   ...    2.62    ...배    (앞 2토큰 = 카운트와 같은 조건)
  → 검증 PPL 최저: v0.1.4(신경망, 2토큰) = 2.62 — 카운트(2.65)를 넘어섬
  ```
