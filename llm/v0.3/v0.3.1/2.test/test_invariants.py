...
spec = importlib.util.spec_from_file_location("v031_test", VERSION / "0.model/lm.py")
...
class Invariants(unittest.TestCase):
    ...
    def test_single_head_with_identity_projection_matches_v030(self):
        single = m._prev.SingleHeadLM(9, 16, 3)
        multi = m.MultiHeadLM(9, 16, 3, 1)
        multi.load_state_dict({**single.state_dict(), "projection.weight": torch.eye(16)})
        x = torch.tensor([[1, 2, 3], [2, 0, 0]])
        torch.testing.assert_close(single(x), multi(x), rtol=0, atol=0)

    def test_split_merge_matches_independent_heads(self):
        net = self.lm.net
        x = torch.tensor([[1, 2, 3], [2, 3, 0]])
        h = net.embedding(x)
        outputs = []
        # 헤드별 독립 행렬곱과 대조해 reshape와 스케일 오류를 검출합니다.
        for i in range(net.heads):
            start, end = i * net.head_dim, (i + 1) * net.head_dim
            q = torch.nn.functional.linear(h, net.query.weight[start:end])
            k = torch.nn.functional.linear(h, net.key.weight[start:end])
            v = torch.nn.functional.linear(h, net.value.weight[start:end])
            score = q @ k.transpose(-1, -2) / math.sqrt(net.head_dim)
            mask = torch.ones(3, 3, dtype=torch.bool).tril() & x.ne(0)[:, None, :]
            outputs.append(score.masked_fill(~mask, -torch.inf).softmax(-1) @ v)
        hidden = net.projection(torch.cat(outputs, dim=-1)) * x.ne(0).unsqueeze(-1)
        expected = net.head(hidden)
        expected[:, :, 0] = -1e9
        torch.testing.assert_close(net(x), expected)
        m.sequence_loss(net(x), torch.tensor([[2, 3, 4], [3, 4, -100]])).backward()
        for layer in (net.query, net.key, net.value):
            for grad in layer.weight.grad.chunk(net.heads, dim=0):
                self.assertGreater(grad.abs().sum().item(), 0)
        self.assertGreater(net.projection.weight.grad.abs().sum().item(), 0)

    def test_head_count_round_trip_and_validation(self):
        for heads in (0, -1, 3):
            with self.assertRaises(ValueError):
                m.MultiHeadLM(9, 16, 3, heads)
        self.lm.HEADS = 2
        self.lm.initialize(self.sentences)
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "model.pt"
            ...
            self.assertEqual(restored.net.head_dim, 8)
            x = torch.tensor([[1, 2, 3]])
            torch.testing.assert_close(self.lm.net(x), restored.net(x), rtol=0, atol=0)
            with self.assertRaises(ValueError):
                m._prev.Model().load(path)
