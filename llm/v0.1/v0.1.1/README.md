...
```python
# 1.data/dataloader.py — 데이터셋 + 로더 생성을 여기에 모아요
...
- **`BigramDataset` + `build_dataloader`** — 데이터셋 클래스와 로더 생성 헬퍼를 **`1.data/dataloader.py`** 에 분리. `Dataset` 은 `__getitem__` 이 **`{'input','target'}` 딕셔너리**를 반환.
