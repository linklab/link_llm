...
| v0.1.5 one-hot 2토큰 신경망 | 3.82 | **31.98** | 46.6% | 83.6% |
| **v0.2.0 임베딩 2토큰 신경망** | 3.40 | 33.39 | **47.6%** | 83.6% |

- 카운트 기준선(34.39)은 **넘었어요** ✅
- **PPL 로는 one-hot(v0.1.5)에 지고, top-1 정확도로는 이깁니다.** 두 지표가 묻는 게 달라서예요 —
  PPL 은 "확률을 얼마나 잘 배분했나", 정확도는 "1등을 얼마나 맞혔나". 임베딩이 후자에서
  앞선다는 게 이 데이터 규모에서 확인되는 '임베딩 효과'예요.
- PPL 로 못 이기는 이유: 학습 문장이 282개뿐이라, "비슷한 토큰끼리 벡터를 공유해 아낀다"는
  임베딩의 장점보다 토큰마다 독립된 칸을 갖는 one-hot 의 **구별력**이 아직 더 큽니다.
