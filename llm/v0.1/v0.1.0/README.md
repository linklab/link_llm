...
**기준은 '학습 손실', `PATIENCE`·`MIN_DELTA` 는 전 버전 통일** — 모든 신경망 버전이 `PATIENCE=20`, `MIN_DELTA=0.0` 을 씁니다.

> ⚠️ **주의 — 학습 손실 기준의 함정.** 학습 손실은 검증과 달리 대체로 계속 내려가서, 조기 종료가 잘 안 걸려
> **검증 최적점을 지나 살짝 과적합**할 수 있어요. 실제로 이 기준으로 재학습하니 신경망들이
> 카운트(검증 PPL 2.96)를 **못 이겼습니다**(2토큰 3.02). 일반화가 목표라면 조기 종료는 **검증**을 보는 게 정석이에요.
> (검증 PPL 은 `loss.svg` 빨강 곡선으로 계속 그려져 비교할 수 있어요.)
