...
## 상속(그대로인 것)
- 임베딩·문맥(block_size)·`make_pairs`·`_context_tensor`·`<PAD>` — v0.2.1/v0.1.4
- 옵티마이저·label smoothing — v0.1.2/1.3 · DataLoader — v0.1.1
- 학습 루프·**조기 종료**·저장·대화·PPL — v0.1.0

## 평가 포인트
`2.test/test.py` 검증 PPL 이 v0.2.1(3.46)과 비슷한 **~3.5**로 나오면 정상이에요 —
쉬운 프로즈에선 이 도구들의 이득이 작거든요(진가는 더 깊고 큰 모델에서).
