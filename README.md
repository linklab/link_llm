...
- [**v0.1.4**](llm/v0.1/v0.1.4/) — **2토큰 문맥 신경망**: 지금까지 앞 1토큰만 보던 신경망을 **앞 2토큰**으로 넓혀
  ...
  softmax 의 부드러움으로 카운트의 FLOOR 절벽을 피해 **검증 PPL 34.39 → 31.98** 로 낮춥니다.
...
- [**v0.1.5**](llm/v0.1/v0.1.5/) — **기준선 대결(캡스톤)**: 같은 학습/검증 데이터로 **v0.0.9(카운트) vs 신경망(1토큰·2토큰)**
  ...
  결과: 1토큰 신경망(39.4)은 카운트(34.39)에 지고, **같은 2토큰**으로 맞춘 v0.1.4~5 가 **31.98** 로 이겨요.
...
- [**v0.2.0**](llm/v0.2/v0.2.0/) — **임베딩 도입**: 입력을 one-hot → **`nn.Embedding`** 으로 바꿔요.
  ...
  결과: **검증 PPL 33.39** — 카운트(34.39)는 넘지만 one-hot(31.98)은 아직 못 이겨요.
...
v0.1.3   신경망       5.29    39.37    23.59   40.7%   83.6%    12.0%    407,065
v0.1.5   신경망       3.82    31.98    18.80   46.6%   83.6%    16.0%    610,842
v0.2.0   신경망       3.40    33.39    19.70   47.6%   83.6%    16.0%    538,650

→ 검증 PPL 최저 : v0.1.5(one-hot)   /   top-1 정확도 최고 : v0.2.0(임베딩)
```

- **PPL 1등과 정확도 1등이 다릅니다.** "확률을 잘 배분하는 것"과 "1등을 잘 맞히는 것"은
  다른 능력이에요 — 어느 모델이 좋다고 말하려면 **어느 자로 쟀는지**를 같이 말해야 합니다.
...
- **생성 다양성**은 PPL 이 전혀 못 보는 것이에요. 온도 0 에서 1토큰 신경망(v0.1.0~3)은
  25개 질문에 **서로 다른 답이 4%** — 사실상 모든 질문에 같은 답을 하는데, PPL 39.37 은
  "좀 못하네" 수준으로만 보입니다.
