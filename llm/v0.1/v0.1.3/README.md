...
> **과적합을 눌러 '처음 보는 것'에 강하게.** 지금 산문 데이터는 쉬워서 학습 vs 검증 격차가 작아요
> (v0.0.9 카운트 기준 학습 2.49 vs 검증 2.96). 그래서 여기서는 정규화의 효과가 크게 드러나진 않지만,
> **진짜 일반화 격차는 나중에 v0.5 SFT** 에서 벌어져요 — 그때 쓸 도구(weight decay·label smoothing)를 미리 익혀 둡니다.
...
`model.pt` 가 생기면 웹앱(`web_service`)에서 v0.1.3 을 골라 **이어쓰기로 바로 평가**할 수 있어요.

## 완결성 — 웹앱에서 바로 평가

**학습 → 생성/이어쓰기 → PPL 측정**까지 한 버전에 완결돼요. 핵심 지표는
