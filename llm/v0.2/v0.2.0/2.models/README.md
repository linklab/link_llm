...
- `model.pt` / `vocab.json` : 학습 결과 (`3.train/train.py` 실행 시 생성). **torch 필요.**

## 이 버전의 목표 — 공유되는 표현(임베딩)
...
`4.test/test.py` 의 검증 PPL 이 v0.1.5(one-hot) 보다 낮아지는지, 나아가 v0.0.9(카운트,
34.39)를 넘는지 보세요. 넘으면 "임베딩으로 신경망이 카운트를 이겼다".
