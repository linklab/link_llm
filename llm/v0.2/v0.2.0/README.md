...
| 모델 | 검증 PPL |
|---|---|
| v0.0.9 카운트 (앞 2토큰 + 백오프) | **2.96** ← 최저 |
| v0.1.5 one-hot 2토큰 신경망 | 3.02 |
| **v0.2.0 임베딩 2토큰 신경망** | 3.06 |

- 카운트 기준선(2.96)을 **아직 못 넘어요** (3.06 > 2.96) — 이 데이터 규모에선 어떤 신경망도 카운트를 못 이깁니다.
- **one-hot(v0.1.5)에도 근소하게 뒤져요** — 3.06 vs 3.02.
  학습 문장이 3,600개인 쉬운 데이터라, "비슷한 토큰끼리 벡터를 공유해 아낀다"는 임베딩의 장점보다
  토큰마다 독립된 칸을 갖는 one-hot 의 **구별력**이 아직 더 크거든요.
  (조기 종료를 **학습 손실(Loss) 기준, patience=20**으로 바꾼 뒤 검증 최적점을 조금 지나쳐 학습해,
  이전 검증-PPL 기준 실행보다 신경망 PPL 이 소폭 올라갔어요.)
