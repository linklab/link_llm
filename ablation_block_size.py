...
[실측 결과 — 문맥은 범인이 아니었어요]
    설정              N=2       N=3      N 효과
    v0.2.0 설정    2.9446    2.9182   -0.0264   ← 넓히니 오히려 좋아짐
    v0.2.1 설정    3.3651    3.3835   +0.0184   ← 미미하게 나빠짐 (방향이 뒤집힘)

문맥 효과는 ±0.02 수준이고 방향도 설정에 따라 달라지는 **잡음**이에요. 반면 용량 차이
(E·H 를 줄인 것)는 2.93 대 3.37 로 **0.43** — 문맥 효과의 20배입니다.
→ v0.2.1 이 뒤진 건 문맥을 넓혀서가 아니라 **함께 줄인 용량 때문**이에요.
...
def report(results):
    ...
    print(f"\n  → 문맥 길이 효과 : 최대 {context_effect:.4f}  "
          f"(부호도 설정에 따라 바뀜 — 사실상 잡음)")
    print(f"  → 용    량 효과 : {capacity_effect:.4f}  "
          f"(E·H 를 줄인 것 — 문맥 효과의 약 {capacity_effect / context_effect:.0f}배)")
    print("\n  ⇒ v0.2.1 이 v0.2.0 보다 뒤지는 건 **문맥을 넓혀서가 아니라 용량을 줄여서**예요.")
    print("    한 번에 하나만 바꿔야 원인을 말할 수 있습니다.")
