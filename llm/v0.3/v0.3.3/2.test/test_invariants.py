...
spec = importlib.util.spec_from_file_location("v033_test", VERSION / "0.model/lm.py")
...
class Invariants(unittest.TestCase):
    ...
    def test_prenorm_residual_and_ffn_paths(self):
        ...
        after_attention = h + net.attention(net.ln1(h), x)
        expected = after_attention + net.ffn(net.ln2(after_attention))
        torch.testing.assert_close(net.transform(h, x), expected)
        m.sequence_loss(net(x), torch.tensor([[2, 3, 4]])).backward()
        for layer in (net.ln1, net.ln2, net.ffn[0], net.ffn[2], net.projection):
            self.assertTrue(torch.isfinite(layer.weight.grad).all())
            self.assertGreater(layer.weight.grad.abs().sum().item(), 0)
        with torch.no_grad():
            net.projection.weight.zero_()
            net.ffn[2].weight.zero_()
            net.ffn[2].bias.zero_()
        torch.testing.assert_close(net.transform(h, x), h, rtol=0, atol=0)
