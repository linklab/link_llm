"""python llm/v0.3/v0.3.4/1.train/train.py --device cpu --epochs 60"""
import argparse
import hashlib
...
def repo_path(value):
    ...
    try:
        ...
        return str(value)
spec = importlib.util.spec_from_file_location("v034", VERSION / "0.model/lm.py")
model = importlib.util.module_from_spec(spec)
spec.loader.exec_module(model)


def main():
    parser = argparse.ArgumentParser(description="v0.3.4 다층 미니 GPT 학습")
    ...
    lm = model.Model()
    for key in ("layers", "ffn_hidden", "dropout", "heads", "epochs", "embed", "block_size", "batch_size", "lr", "patience", "seed", "device"):
    ...
    report = {"version": "v0.3.4", "config": {k: repo_path(v) if k in ("train", "valid", "output_dir") else v
