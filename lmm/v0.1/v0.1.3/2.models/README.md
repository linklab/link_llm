...
v0.0.9 에서 **학습 PPL 2.79 vs 검증 PPL 34.39**(약 12배). 신경망도 그냥 두면 "본 것만 잘 맞히는"
과적합이 생겨요. v0.1.3 은 **과신을 눌러** 처음 보는 조합에도 덜 틀리게 만듭니다.
...
- **하이퍼파라미터** (`3.train/train.py` 에서 설정): `WEIGHT_DECAY=1e-4`, `INIT="zeros"`, `LABEL_SMOOTHING=0.0` (+ `OPTIMIZER/LR/EPOCHS/BATCH_SIZE`).
...
- 평가 핵심: `4.test/test.py` 의 **학습/검증 PPL 격차**가 줄었나 (그리고 v0.1.4 에서 v0.0.9 와 비교).
