...
| dropout | 2.9081 | 281,865 | +0.033 |
| tying | 2.9253 | **166,025** | +0.051 |
| cosine | 2.8716 | 281,865 | **−0.003** |
| all | 2.9376 | **166,025** | +0.063 |

- 가장 큰 도구 효과가 **0.063** 인데, 도구를 하나도 안 건드리고 `EPOCHS` 만 1,500→300 으로
  바꾸면 **0.33** 이 개선돼요(2.96 → 2.63, v0.2.5). **설정 하나가 도구 전부보다 5배 큽니다.**
- **신호 < 잡음** → 이 표로는 도구의 우열을 말할 수 없습니다. 부호도 실행마다 뒤집혀요.
- 원인은 `EPOCHS` 가 cosine 감쇠 기간(`T_max`)을 겸하는 구조 —
  자세한 설명은 [버전 README](../README.md#왜-흔들리나--epochs-가-두-가지-일을-겸해서).
- 흔들리지 않는 유일한 사실: **weight tying 은 파라미터를 41% 줄인다**(281,865 → 166,025).
  측정이 아니라 구조에서 나오는 값이라 흔들리지 않아요.
